...
describe("HomeComponent", () => {
  ...
  const beginnerCourses = setupCourses().filter(
    (course: Course) => course.category.toUpperCase() === "BEGINNER"
  );

  const advancedCourses = setupCourses().filter(
    (course: Course) => course.category.toUpperCase() === "ADVANCED"
  );
  ...
  it("should display only beginner courses", () => {
    ...
    expect(tabs.length).toBe(1, "Expected to find only 1 tab.");
  });
